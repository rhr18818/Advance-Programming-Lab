...
# known_face_names = []
# # Load images from 'image' folder and encode the faces
# for filename in os.listdir("image"):
#     path = os.path.join("image", filename)
#     img = face_recognition.load_image_file(path)
#     encodings = face_recognition.face_encodings(img)
    
#     if encodings:  # make sure there's a face found
#         known_face_encodings.append(encodings[0])
#         name = os.path.splitext(filename)[0]  # Use file name as the person’s name
...
#         # Scale back up face locations since we resized the frame earlier
#         top *= 4
#         right *= 4
#         bottom *= 4
#         left *= 4
